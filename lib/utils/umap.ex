defmodule UMap do
  @moduledoc """
  Tuple-based UMap implementation.
  """
  alias Ippan.Dict
  use Ippan.Dict
  ...
  def put(%UMap{root: root, size: size}, key, value) do
    {root, counter} = do_put(root, key, value, key_hash(key))
    %UMap{root: root, size: size + counter}
  end

  def update!(%UMap{root: root, size: size} = dict, key, fun) when is_function(fun, 1) do
    {root, counter} =
  ...
  def update(%UMap{root: root, size: size}, key, default, fun) when is_function(fun, 1) do
    {root, counter} = do_update(root, key, fn -> default end, fun, key_hash(key))
    %UMap{root: root, size: size + counter}
  end

  def get(%UMap{root: root}, key) do
    do_get(root, key, key_hash(key))
  end

  def get(%UMap{root: root}, key, default) do
    do_get(root, key, key_hash(key)) || default
  end

  def fetch(%UMap{root: root}, key) do
  ...
  def size(%UMap{size: size}) do
    size
  end

  @doc false
  def reduce(%UMap{root: root}, acc, fun) do
  ...
  @doc false
  def dict_delete(%UMap{root: root, size: size}, key) do
    case do_delete(root, key, key_hash(key)) do
      {root, value} -> {%UMap{root: root, size: size - 1}, value}
      :error -> :error
    end
  end
  ...
  defp do_get(node, key, hash) do
    index = key_mask(hash)

    case elem(node, index) do
      [^key | v] -> v
      {^key, v, _} -> v
      {_, _, n} -> do_get(n, key, key_shift(hash))
      _ -> nil
    end
  end
...
defimpl Enumerable, for: UMap do
  @module UMap
...
defimpl Collectable, for: UMap do
  @module UMap
...
defimpl Inspect, for: UMap do
  import Inspect.Algebra
  @module UMap

  def inspect(dict, opts) do
    concat(["#UMap<", Inspect.List.inspect(@module.to_list(dict), opts), ">"])
  end
end
