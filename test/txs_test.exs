defmodule TransactionsTest do
  ...
  test "send txs" do
    ...
    {:ok, c1} =
      Builder.coin_new(c1, "IPN", [[c2.id, 10_000_000], [c3.id, 10_000_000]]) |> post(hostname)

    {:ok, c1} = Builder.coin_new(c1, "IPN", [[c2.id, 1_000_000]]) |> post(hostname)

    {:ok, c1} =
      Builder.coin_new(c1, "IPN", [["0xziUv22NNVEPt8ikCGb7xip2CPgB", 1_000_000]])
      |> post(hostname)

    {:ok, c1} =
      Builder.coin_new(c1, "IPN", [["0x2gosNPWWeLbJfAx5aMnqTuXyYZSv", 400_000]]) |> post(hostname)

    # Tokens
    {:ok, c2} =
      Builder.token_new(c2, "USD", c2.id, "Dollar", 2, "$", 0, %{
        "avatar" => "https://ippan.com/images/USD.png",
        "props" => ["coinbase", "lock", "burn", "drop"],
        "env" => %{}
      })
      |> post(hostname)

    {:ok, c2} = Builder.token_update(c2, "USD", %{"name" => "Dollar"}) |> post(hostname)
    {:ok, c2} = Builder.coin_new(c2, "USD", [[c2.id, 1_000_000]]) |> post(hostname)

    {:ok, c3} =
      Builder.token_new(c3, "EUR", c3.id, "Euro", 2, "€", 0, %{
        "avatar" => "https://ippan.com/images/EUR.png",
        "props" => ["coinbase", "lock", "burn", "drop"],
        "env" => %{}
      })
      |> post(hostname1)
    ...
    {:ok, c3} =
      Builder.token_new(c3, "GBP", c3.id, "Pound", 2, "£", 0, %{
        "avatar" => "https://ippan.com/images/GBP.png",
        "props" => ["coinbase", "lock", "burn", "drop"],
        "env" => %{}
      })
      |> post(hostname1)
    ...
    {:ok, c1} =
      Builder.token_update(c1, "XPN", %{"avatar" => "https://ippan.com/images/XPN2.png"})
      |> post(hostname)
    ...
    {:ok, c2} =
      Builder.coin_refund(c2, "537163378D903B3E1C631B767EF7C33916C920FCCFE54402750F60AFE4855CBD")
      |> post(hostname)

    {:ok, c2} =
      Builder.coin_multisend(c2, "USD", [[c1.id, 1000], [c3.id, 2000]], "Multisend")
      |> post(hostname)

    {:ok, c2} =
      Builder.domain_new(c2, "example.ipn", c2.id, 2, %{
    ...
    {:ok, c2} =
      Builder.domain_new(c2, "example2.ipn", c2.id, 2, %{
        "email" => "name@example.com",
        "avatar" => "https://avatar.com"
      })
      |> post(hostname)
    ...
    {:ok, c2} =
      Builder.domain_new(c2, "example3.ipn", c2.id, 2, %{
        "email" => "name@example.com",
        "avatar" => "https://avatar.com"
      })
      |> post(hostname)
    ...
    {:ok, c2} =
      Builder.domain_new(c2, "example4.ipn", c2.id, 2, %{
        "email" => "name@example.com",
        "avatar" => "https://avatar.com"
      })
      |> post(hostname)
    ...
    {:ok, c2} =
      Builder.domain_update(c2, "example.ipn", %{"email" => "pop@email.com"}) |> post(hostname)
    ...
    {:ok, c2} =
      Builder.dns_update(c2, "example.ipn", "EA239F6544AC1EB1AE0A9DE3FEDB2BAC", %{
        "data" => "192.168.0.2"
      })
      |> post(hostname)
    ...
    {:ok, c1} =
      Builder.validator_update(c1, 1, %{"owner" => "0x2gosNPWWeLbJfAx5aMnqTuXyYZSv"})
      |> post(hostname)
    ...
    c2 =
      Enum.reduce_while(1..1000, c2, fn _, x ->
    ...
    c4 =
      Enum.reduce_while(1..1000, c4, fn _, x ->
        monto = 10 + :rand.uniform(200)
        IO.puts("#{x.nonce} - monto: #{monto}")
    ...
    c2 =
      Enum.reduce_while(1..50, c2, fn _, x ->
        monto = 10 + :rand.uniform(500)
        IO.puts("#{x.nonce} - monto: #{monto}")

        case Builder.coin_send(x, "0xziUv22NNVEPt8ikCGb7xip2CPgB", "USD", monto, "random c2")
             |> post(hostname) do
          {:ok, x} ->
            {:cont, x}
    ...
    c3 =
      Enum.reduce_while(1..2000, c3, fn _, x ->
    ...
    c2 =
      Enum.reduce_while(1..5, c2, fn _, y ->
        dominio = RandomString.generate() <> ".ipn"
        IO.puts("#{y.nonce} - DOMINIO: #{dominio}")

        case Builder.domain_new(c2, dominio, c2.id, 2, %{
               "email" => "name@example.com",
               "avatar" => "https://avatar.com"
             })
             |> post(hostname) do
    ...
    c3 =
      Enum.reduce_while(1..100, c3, fn _, x ->
        monto = 10 + :rand.uniform(500)
        IO.puts("#{x.nonce} - monto: #{monto}")
